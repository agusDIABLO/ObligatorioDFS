...
export const validateCustomerAvailabilityMiddleware = async (req, res, next) => {
    try {
        const { serviceId, reservationDate, reservationTime } = req.body;
        ...
        // Buscar reservas existentes del cliente en la misma fecha
        const existingReservations = await reservationRepository.getReservationByCustomerAndDate(customerId, reservationDate);
