...
const reservationMongoRepository = {
    ...
    async getReservationByBarberAndDate(barberId, startOfDay, endOfDay) {
        try {
            ...
            throw new Error(
                "error al obtener reservas por fecha y barbero en la base de datos"
            );
        }
    },

    async getReservationsByCustomerAndDateRange(
        customerId,
        startOfDay,
        endOfDay
    ) {
        try {
            ...
            throw new Error(
                "error al obtener reservas por fecha y cliente en la base de datos"
            );
        }
    },
    ...
    async getReservationById(id) {
        try {
            const reservation = await Reservation.findById(id)
                .populate("serviceId", "duration name")
                .lean();
            return reservation;
        } catch (error) {
